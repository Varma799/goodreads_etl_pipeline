...
### Hardware Used
EMR - I configured a 3 node cluster with the below Instance Types:

    m5.xlarge
    4 vCore, 16 GiB memory, EBS only storage
    EBS Storage: 64 GiB
Redshift: For Redshift I utilized a 2 Node cluster with Instance Types "dc2.large"

### Setting Up Airflow

I have provided detailed instructions on how to setup Airflow using AWS CloudFormation scripts. Check out the documentation for - [Airflow using AWS CloudFormation](https://github.com/san089/Data_Engineering_Projects/blob/master/Airflow_Livy_Setup_CloudFormation.md)
...
The project uses "sshtunnel" to submit spark jobs using a ssh connection from the EC2 instance. This setup does not automatically install "sshtunnel" for apache airflow. You can install by running the below command: 

    pip install apache-airflow[sshtunnel]

Finally, copy the dag and plugin folder to EC2 inside the airflow home directory. Also, checkout [Airflow Connection](https://github.com/san089/goodreads_etl_pipeline/blob/master/docs/Airflow_Connections.md) for setting up connection to EMR and Redshift from Airflow.
...
ETL jobs in the project use [psycopg2](https://pypi.org/project/psycopg2/) to connect to the Redshift cluster to run staging and warehouse queries. 
To install psycopg2 on EMR:

    sudo pip-3.6 install psycopg2

psycopg2 uses "postgresql-devel" and "postgresql-libs", and sometimes psycopg2 installation may fail if these dependencies are not available. To install run commands:
...
Finally, pyspark uses python2 as the default setup on EMR. To change to python3, setup environment variables:

    export PYSPARK_DRIVER_PYTHON=python3
    export PYSPARK_PYTHON=python3

Copy the ETL scripts to EMR and the environment is ready to run jobs. 

### Setting up Redshift
You can follow the AWS Guide to run a Redshift cluster or alternatively you can use the [Redshift_Cluster_IaC.py](https://github.com/san089/Data_Engineering_Projects/blob/master/Redshift_Cluster_IaC.py) script to create the cluster automatically. 
...
## Testing the Limits
The "goodreadsfaker" module in this project generates fake data which is used to test the ETL pipeline under heavy load.  

To test the pipeline, I used "goodreadsfaker" to generate 11.4 GB of data to be processed every 10 minutes (including ETL jobs + populating data into warehouse + running analytical queries) by the pipeline, which equates to around 68 GB/hour and about 1.6 TB/day.
...
- Pipelines would be run on 7am daily. how to update dashboard? would it still work?
    ...
    - Data quality operators are used at appropriate positions. In case of DAG failures, email triggers can be configured to notify the team.
    
- Make it available to 100+ people
    - We can set the concurrency limit for the Amazon Redshift cluster. While the concurrency limit is 50 parallel queries for a single period of time, this is on a per cluster basis, meaning you can launch as many clusters as fit for the business requirements.
...
I am a Senior Data Engineer with 6+ years of experience designing ETL pipelines, lakehouse platforms, warehouses, and analytics solutions across Azure, AWS, and GCP. I specialize in Python, SQL, Spark, Databricks, Snowflake, Airflow, and cloud storage, delivering reliable batch, streaming, and orchestration capabilities for production workloads consistently at scale. This project represents a robust implementation of a cloud-native data pipeline designed for high-volume processing and analytical integrity.